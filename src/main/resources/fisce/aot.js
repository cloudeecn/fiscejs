...
var FyAOTUtil = new __FyAOTUtil({
	"ops" : {
		...
		"AASTORE" : {
			"code" : "lip = $ip;ops--;sp -= 3;if (stack[sp] !== 0|| !context.classLoader.canCast(heap.getClassFromHandle(stack[0]), heap.getClassFromHandle(stack[2]).contentClass)) {throw new FyException(FyConst.FY_EXCEPTION_STORE,\"Can't store \"+ heap.getClassFromHandle(stack[0]).name+ \" to \"+ heap.getClassFromHandle(stack[2]).name);}heap.putArrayInt(stack[2], stack[1], stack[0]);",
			"pops" : "-3",
			"pushes" : "0"
		},
		"FASTORE" : {
			"code" : "lip = $ip;ops--;sp -= 3;heap.putArrayInt(stack[2], stack[1], stack[0]);",
			"pops" : "-3",
			"pushes" : "0"
		},
		"IASTORE" : {
		...
		"PUTSTATIC" : {
			"code" : "lip = $ip;ops--;tmpField = context.lookupFieldVirtualFromConstant(constant[$1]);if (tmpField.accessFlags & FyConst.FY_ACC_STATIC == 0) {throw new FyException(FyConst.FY_EXCEPTION_INCOMPAT_CHANGE, \"Field \"+ tmpField.uniqueName+ \" is not static\");}if ((tmpField.accessFlags & FyConst.FY_ACC_FINAL)&& (method.owner != tmpField.owner)) {throw new FyException(FyConst.FY_EXCEPTION_ACCESS,\"Field \" + tmpField.uniqueName + \" is final\");}clinitClass = thread.clinit(tmpField.owner);if (clinitClass !== undefined) {if (clinitClass.clinitThreadId == 0) {thread.localToFrame(sp, $ip, $ip);thread.pushFrame(clinitClass.clinit);return ops;} else {ops = 0;ip = $ip;break __fy_outer;}}switch (tmpField.descriptor.charCodeAt(0)) {case FyConst.D:case FyConst.J:sp -= 3;heap.putStaticRawLongFrom(stack[sp], tmpField.posAbs,stack, sp + 1);break;default:sp -= 2;heap.putStaticRaw(stack[sp], tmpField.posAbs,stack[sp]);break;}",
			"pops" : "X-PUTSTATIC",
			"pushes" : "0"
		},
		"RETURN" : {
	...
	"macros" : {
		"HEADER" : "var clazz = this.owner;var heap = context.heap;var context = thread.context;var longOps = thread.longOps;var constants = clazz.constants;var stack = thread.stack;var floatStack = thread.floatStack;var ip = thread.getCurrentIp() | 0;var lip = 0 | 0;var sp = thread.sp | 0;var sb = thread.getCurrentStackBase() | 0;var tmpField;var tmpClass;var clinitClass;var tmpMethod;var tmpInt1 = 0 | 0;__fy_outer: while (true) {try {__fy_inner: switch (ip) {",
		"OPS" : "if (ops < 0) {lip = $ip;ip = $ip;break __fy_outer;}",
		"TAIL" : "default:throw new FyException(undefined, \"IP out of sync at \"+ this.uniqueName + \".\" + ip);}} catch (e) {if (e instanceof FyException) {(function() {if (!e.clazz) {context.panic(e.toString());throw e;}try {var exceptionClass = context.lookupClass(e.clazz);if (context.classLoader.canCast(exceptionClass,context.TOP_THROWABLE)) {throw new FyException(undefined, \"Exception \"+ exception.clazz+ \" is not a java.lang.Throwable\");}var detailMessageField = context.getField(FyConst.FY_BASE_THROWABLE+ \".detailMessage.L\"+ FyConst.FY_BASE_STRING);thread.currentThrowable = heap.allocate(context.lookupClass(e.clazz));heap.putFieldString(thread.currentThrowable,detailMessageField, e.message);thread.fillStackTrace(thread.currentThrowable,false);break __fy_outer;} catch (ee) {context.panic(\"Exception occored while processing exception: \"+ e);throw ee;}})();} else {context.panic(\"Exception occored while executing thread #\"+ thread.threadId);throw e;}}}thread.localToFrame(sp, lip, ip);"
	}
});
