(function(window) {
		...
		for (var leftLPos = 0; leftLPos < len; leftLPos++) {
			...
			for (var rightHPos = 0; rightHPos < len; rightHPos++) {
				...
				for (var rightLPos = 0; rightLPos < len; rightLPos++) {
					...
					pos = leftHPos * len * len * len + leftLPos * len * len + rightHPos * len + rightLPos;
					values[pos << 2] = leftH;
	...
	function testLong(op) {
		...
		for (pos = 0; pos < vlen; pos++) {
			...
			// console.log(op + "(" + stack[17] + ", " + stack[18] + " ,,, " + stack[19] + ", " + stack[20] + ")")
			lop[op](17, 19);

			var targetResult = resultData[pos];
			if (stack[17] != targetResult[0] || stack[18] != targetResult[1]) {
				failed = true;
				ok(false, "long [" + values[(pos << 2) + 0] + ", " + values[(pos << 2) + 1] + "] " + op + " [" + values[(pos << 2) + 2] + ", " + values[(pos << 2) + 3] + "] === [" + stack[17] + ", " + stack[18] + "] but correct one is [" + targetResult[0] + ", " + targetResult[1] + "]");
			}
		}
		if (!failed) {
			ok(true, "long " + op);
	...
	fisceTests.extend({
		...
		"   Long benchmark": function() {
			...
			time = end - begin;
			ok(true, times * vlen * 10 + " int adds costs " + time + "ms" + intValue);

			begin = performance.now();
			for (var i = 0; i < times * vlen; i++) {
				FyPortable.doubleToIeee64(FyPortable.ieee64ToDouble(stack, 2) + FyPortable.ieee64ToDouble(stack, 4), stack, 6);
				FyPortable.doubleToIeee64(FyPortable.ieee64ToDouble(stack, 2) + FyPortable.ieee64ToDouble(stack, 4), stack, 6);
				FyPortable.doubleToIeee64(FyPortable.ieee64ToDouble(stack, 2) + FyPortable.ieee64ToDouble(stack, 4), stack, 6);
				FyPortable.doubleToIeee64(FyPortable.ieee64ToDouble(stack, 2) + FyPortable.ieee64ToDouble(stack, 4), stack, 6);
				FyPortable.doubleToIeee64(FyPortable.ieee64ToDouble(stack, 2) + FyPortable.ieee64ToDouble(stack, 4), stack, 6);
			...
			time = end - begin;
			ok(true, times * vlen * 10 + " doubles adds costs " + time + "ms " + doubleValue);

			begin = performance.now();
			for (var i = 0; i < times * vlen; i++) {
				stack[2] = FyPortable.floatToIeee32(FyPortable
					.ieee32ToFloat(stack[0]) + FyPortable.ieee32ToFloat(stack[1]));
				stack[2] = FyPortable.floatToIeee32(FyPortable
					.ieee32ToFloat(stack[0]) + FyPortable.ieee32ToFloat(stack[1]));
				stack[2] = FyPortable.floatToIeee32(FyPortable
					.ieee32ToFloat(stack[0]) + FyPortable.ieee32ToFloat(stack[1]));
			...
			time = end - begin;
			ok(true, times * vlen * 10 + " floats adds costs " + time + "ms " + doubleValue);
			...
			for (var opid in ops) {
				...
				time = end - begin;
				ok(true, "long : " + times * vlen * 10 + " longs " + op + " costs " + time + "ms " + stack[17] + " " + stack[19]);
			}
