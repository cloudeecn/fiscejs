...
public class LZString {
	...
	static {
		for (int i = 0, max = keyStr.length(); i < max; i++) {
			if (i < 64) {
				codeMap[keyStr.charAt(i)] = i;
			} else {
				codeMap[keyStr.charAt(i)] = 0;
			}
		}
	}

	public static StringBuilder compress(CharSequence uncompressed,
			StringBuilder out) {
					...
					if (((int) context_w.charAt(0)) < 256) {
						for (int i = 0; i < context_numBits; i++) {
							...
							if (context_data_position == 15) {
								...
								out.append((char) context_data_val);
								context_data_val = 0;
							} else {
	...
	public static String decompress(CharSequence compressed) {
	...
	public static StringBuilder encodeUTF16(CharSequence input,
			StringBuilder out) {
		int c, status = 0, current = 0;
	...
	public static StringBuilder compressToUTF16(CharSequence input,
			StringBuilder out) {
		if (out == null) {
			out = new StringBuilder();
		}
		...
		LZString.compress(input, compressResult);
		encodeUTF16(compressResult, out);
		return out;
	}

	public static StringBuilder decodeUTF16(CharSequence input,
	...
	public static String decompressFromUTF16(CharSequence input) {
		if (input == null)
			return "";
		StringBuilder output = new StringBuilder(200);
		LZString.decodeUTF16(input, output);
		return LZString.decompress(output);
		// return output;

	}

	public static String decompressFromBase64(CharSequence input)
			throws Exception {
		return LZString.decompress(decode64(input));
	}

	// implemented from JS version
	public static String decode64(CharSequence input) {
		...
		while (i < input.length()) {
			...
			enc1 = codeMap[input.charAt(i++)];
			enc2 = codeMap[input.charAt(i++)];
			enc3 = codeMap[input.charAt(i++)];
			enc4 = codeMap[input.charAt(i++)];
	...
	public static void encode64(CharSequence input, StringBuilder result) {
	...
	public static void compressToBase64(CharSequence input, StringBuilder out) {
		StringBuilder compressedOutput = new StringBuilder();
		compress(input, compressedOutput);
		encode64(compressedOutput, out);
	}
	...
	public static void main(String args[]) throws Exception {
		...
		LZString.compress(test, out);

		System.out.println("Compressed: " + out);

		String decompressed = LZString.decompress(out);
		...
		LZString.compressToUTF16(testUTF16, out);

		System.out.println("Compressed: " + out);

		String decompressedUTF16 = LZString.decompressFromUTF16(out);
		...
		LZString.compressToBase64(testBase64, out);

		System.out.println("Compressed: " + out);

		String decompressedBase64 = LZString.decompressFromBase64(out);

		System.out.println("Decompressed: " + decompressedBase64);
		StringBuilder in = new StringBuilder(64);
		StringBuilder out2 = new StringBuilder(64);
		for (int i = 0; i < 64; i++) {
			in.append((char) 0xaa55);
			out.setLength(0);
			encodeUTF16(in, out);
			System.out.println(in + " => " + out);
			out2.setLength(0);
			decodeUTF16(out, out2);
			System.out.println(out2 + " <= " + out);
			System.out.println(in.toString().equals(out2.toString()));
		}
	}
}
